...
<template>
  <div class="partner-section my-16">
    <h2 class="text-center text-2xl font-bold">合作伙伴</h2>
    <UCarousel
      ref="carouselRef"
      :items="partners || []"
      :ui="{ item: 'basis-1/3 px-0' }"
      class="mt-8 group relative"
      wrap-around
      arrows
      ...
      <template #default="{ item }">
        <NuxtLink :to="item.url" class="block px-0 relative group/item">
          <div class="relative">
            ...
            />
            <div class="absolute inset-0 bg-black/50 opacity-0 group-hover/item:opacity-100 transition-opacity flex flex-col items-center justify-center gap-2">
